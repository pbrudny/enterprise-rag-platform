...
A working practice implementation exists, per README.md: a Python core library (`src/rag_platform/`) exposed both as a CLI (`rag ...`) and an HTTP API (`src/rag_platform/api/`, FastAPI), plus a React/TypeScript web frontend (`frontend/`), deployed to a Mikrus VPS via Coolify behind a Basic Auth gate. Providers are pluggable — local/OpenAI/Anthropic/**real Vertex AI** for embeddings+generation, local or **remote Chroma** for the vector store — selected via `~/agenty/secrets/.env`. Document ingestion is available both from the CLI and the web UI (`/documents`, scoped to manager/security_admin roles, capped at the caller's own tenant/clearance/ACL membership). This is still a practice/interview-prep build, not a production deployment of the PRD below; see README.md for how to run the CLI, API, and frontend, and the scope-cuts noted there (no real SSO).
